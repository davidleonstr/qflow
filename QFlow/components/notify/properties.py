...
STYLEPATH = Source.library(r'QFlow/components/notify/notify.qss').get()
"""Path to the file containing the styles for the Notify object."""

# References to create the default icons of the Notify class
ICONS = {
    'success': lambda: Icon(
        Source.library(r'QFlow/components/notify/icons/check.png').get(), 
        25, 25
    ),
    'error': lambda: Icon(
        Source.library(r'QFlow/components/notify/icons/close.png').get(), 
        25, 25
    ),
    'info': lambda: Icon(
        Source.library(r'QFlow/components/notify/icons/information.png').get(), 
    25, 25),
}
"""
