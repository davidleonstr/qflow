...
from qtpy.QtWidgets import QLabel, QVBoxLayout, QWidget, QProgressBar, QHBoxLayout, QFrame
from qtpy.QtGui import QPixmap
from qtpy.QtCore import QTimer, Qt
from typing import List, Dict

# Decorator for applying styles to PyQt-PySide widgets
from QFlow.injectors.style import style

# Importing style properties and configuration
from .properties import STYLEBAR, STYLEPATH, THEME, ICONS
...
class Notify(QWidget):
    ...
    def __init__(
        ...
        self.animationValues.update(animationValues)

        self.animatedEvents = {
            'fadeIn': True,
            'fadeOut': True
        }
        self.animatedEvents.update(animatedEvents)
        ...
        if color in THEME:
            self.containerStyle = THEME[color]['QFrame']
        else:
        ...
        if color in THEME:
            self.messageLabel.setObjectName(THEME[color]['QLabel'])
        else:
            raise KeyError(f"The color does not exist in Notify: '{color}'.")
    ...
    def show(self) -> None:
        ...
        if self.delay > 0:
            ...
            QTimer.singleShot(self.delay, self.showNotification)
        else:
    ...
    def animateFadeOut(self) -> None:
        timer = QTimer(self)
        opacity = self.windowOpacity()

        def modifyOpacity():
            nonlocal opacity
            opacity -= self.animationValues['opacityReductionOut']
        ...
        timer.timeout.connect(modifyOpacity)
        timer.start(self.msRenderTime)

    def animateFadeIn(self) -> None:
        ...
        def modifyOpacity():
            nonlocal opacity
            opacity += self.animationValues['opacityIncreasedIn']
    ...
    def updateProgress(self) -> None:
        ...
        if self.elapsedTime >= self.duration:
            self.timer.stop()
            if self.animatedEvents['fadeOut']:
                self.animateFadeOut()
